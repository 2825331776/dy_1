...
public class UVCCamera {
	...
	public synchronized void open (final UsbControlBlock ctrlBlock) {
		...
		try {
			...
			result = nativeConnect(mNativePtr, mCtrlBlock.getVenderId(), mCtrlBlock.getProductId(), mCtrlBlock.getFileDescriptor(), mCtrlBlock.getBusNum(), mCtrlBlock.getDevNum(), getUSBFSName(mCtrlBlock));
...
//		nativeSetPreviewSize(mNativePtr, DEFAULT_PREVIEW_WIDTH, DEFAULT_PREVIEW_HEIGHT, DEFAULT_PREVIEW_MIN_FPS, DEFAULT_PREVIEW_MAX_FPS, DEFAULT_PREVIEW_MODE, DEFAULT_BANDWIDTH, platform);
	}
	...
	public void setPreviewSize(final int width, final int height, final int currentAndroidVersion) {
		setPreviewSize(width, height, DEFAULT_PALETTE_TYPE, DEFAULT_PREVIEW_MIN_FPS, DEFAULT_PREVIEW_MAX_FPS, mCurrentFrameFormat, mCurrentBandwidthFactor,currentAndroidVersion);
	}
	 */
	...
	public void setPreviewSize(final int width, final int height, final int frameFormat,final int currentAndroidVersion) {
		setPreviewSize(width, height, DEFAULT_PALETTE_TYPE, DEFAULT_PREVIEW_MIN_FPS, DEFAULT_PREVIEW_MAX_FPS, frameFormat, mCurrentBandwidthFactor,currentAndroidVersion);
	}
	...
	public void setPreviewSize(final int width, final int height, final int frameFormat, final float bandwidth,final int currentAndroidVersion) {
		setPreviewSize(width, height, DEFAULT_PALETTE_TYPE, DEFAULT_PREVIEW_MIN_FPS, DEFAULT_PREVIEW_MAX_FPS, frameFormat, bandwidth,currentAndroidVersion);
	}
	 */
	...
	public void setFrameCallback (final IFrameCallback callback, final int pixelFormat) {
		if (mNativePtr != 0) {
			//			nativeSetFrameCallback(mNativePtr, callback, pixelFormat);  //2021年8月11日16:30:43 注释 吴长城
		}
	}
	...
	//================================================================================
	public void testJNi(String phoneModel){
		if (mNativePtr != 0) {
			nativeTestJNI(mNativePtr,phoneModel);
		}else {
	...
	public boolean setMachineSetting (final int flag, final int value, final int mark) {
		Log.e(TAG, "setMachineSetting:  flag = " + flag + " value = " + value + " mark " + mark);
		boolean result = false;
		if (mNativePtr != 0) {
			result = nativeSetMachineSetting(mNativePtr, flag, value, mark);
		}
		return result;
	}
	...
	public float getMachineSetting (final int flag, final int value, final int mark) {
		Log.e(TAG, "getMachineSetting:  flag = " + flag + " value = " + value + " mark " + mark);
		float result = 0;
		if (mNativePtr != 0) {
			result = nativeGetMachineSetting(mNativePtr, flag, value, mark);
		}
		return result;
	}
	...
	public void startCapture () {
		if (mCtrlBlock != null) {
			...
			//			nativeStartStopCapture(mNativePtr, 1);//2021-8-11 16:21:28 吴长城注释
		}
	}
	...
	public void stopCapture () {
		if (mCtrlBlock != null) {
			...
			//			nativeStartStopCapture(mNativePtr, 0);//2021-8-11 16:21:28 吴长城注释
		}
	}
	...
	public void setPath (String path) {
		if (mCtrlBlock != null) {
			Log.e(TAG, "setPath");
			nativeSetAddress(mNativePtr, path);
		}
	}

	public void setPalette (int type) {
		if (mCtrlBlock != null) {
			Log.e(TAG, "setPalette");
			nativeChangePalette(mNativePtr, type);
		}
	}

	public void setAreaCheck (int isAreaCheck) {
		if (mCtrlBlock != null) {
			Log.e(TAG, "setAreaCheck");
			nativeSetAreaCheck(mNativePtr, isAreaCheck);
		}
	}
	...
	public void SaveFiveSeconds (String path) {//长城 :保存五帧数据
		if (mCtrlBlock != null) {
			//			Log.e(TAG, "SaveFiveSeconds");
			nativeSaveFiveSecondsData(mNativePtr, path);
		}
	}
	...
	public void changePalette (int typeOfPalette) {
		if (mCtrlBlock != null) {
			Log.e(TAG, "changePalette" + typeOfPalette);
	...
	//2022年5月17日16:29:30 获取机芯设置参数
	private static final native float nativeGetMachineSetting (final long id_camera, final int flag, final int value, final int mark);

	//2022年5月17日16:29:30 设置机芯设置参数
	private static final native boolean nativeSetMachineSetting (final long id_camera, final int flag, final int value, final int mark);
	...
	private static final native int nativeCallJniSavePreViewPicture (final long id_camera, final String filePath, final int preViewWidth, final int preViewHeight);//调用JNI保存预览画面的图片方法
	...
	private static final native void nativeSetAddress (final long id_camera, String path);
	...
	//测试JNI 通道
	private static final native void nativeTestJNI(final long id_camera, final String str);
