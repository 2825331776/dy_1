...
public final class USBMonitor {
	...
	private final BroadcastReceiver mUsbReceiver = new BroadcastReceiver() {
		...
		public void onReceive (final Context context, final Intent intent) {
			...
			if (ACTION_USB_PERMISSION.equals(action)) {
				if (DEBUG)
					Log.e(TAG, "=====register============before==========BroadcastReceiver mUsbReceiver processConnect device" +
							"=======");
				...
				synchronized (USBMonitor.this) {
					...
					if (intent.getBooleanExtra(UsbManager.EXTRA_PERMISSION_GRANTED, false)) {
						if (device != null) {
							...
							if (DEBUG)
								Log.e(TAG, "UsbMonitor.register.broadcastReceiver.mUsbReceiver.OnReceiver=device !=null!!!====todo " +
										"connect");
							processConnect(device);
						}
					} else {
	...
	public static class UsbDeviceInfo {
		...
		public String toString () {
			return String.format("UsbDevice:usb_version=%s,manufacturer=%s,product=%s,version=%s,serial=%s", usb_version != null ?
					usb_version : "", manufacturer != null ? manufacturer : "", product != null ? product : "", version != null ? version
					: "", serial != null ? serial : "");
		}
	}
	...
	private static String getString (final UsbDeviceConnection connection, final int id, final int languageCount, final byte[] languages) {
		...
		for (int i = 1; i <= languageCount; i++) {
			int ret = connection.controlTransfer(USB_REQ_STANDARD_DEVICE_GET, // USB_DIR_IN | USB_TYPE_STANDARD | USB_RECIP_DEVICE
					USB_REQ_GET_DESCRIPTOR, (USB_DT_STRING << 8) | id, languages[i], work, 256, 0);
		...
		if (device != null) {
			...
			if ((manager != null) && manager.hasPermission(device)) {
				...
				if (connection != null) {
					...
					try {
						int result = connection.controlTransfer(USB_REQ_STANDARD_DEVICE_GET, // USB_DIR_IN | USB_TYPE_STANDARD |
								// USB_RECIP_DEVICE
								USB_REQ_GET_DESCRIPTOR, (USB_DT_STRING << 8) | 0, 0, languages, 256, 0);
						if (result > 0) {
