...
static jbyteArray nativeGetByteArrayTemperaturePara(JNIEnv *env, jobject thiz,
	...
	uint8_t *para=(uint8_t *)malloc(len*sizeof(uint8_t));//通过传入的长度分配了一个len*8个字节的内存。也就是len个长度的char
	int status=0;
	if (LIKELY(camera)) {
...
//长城添加:保存五秒数据
static void nativeSaveFiveSecondsData(JNIEnv *env, jobject thiz,ID_TYPE id_camera, jstring externalPath){
    ENTER();
    UVCCamera *camera = reinterpret_cast<UVCCamera *>(id_camera);
    if (LIKELY(camera)) {
