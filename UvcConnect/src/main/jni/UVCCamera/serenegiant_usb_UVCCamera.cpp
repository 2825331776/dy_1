...
static void
nativeSetRotateMatrix_180(JNIEnv *env, jobject thiz, ID_TYPE id_camera, jboolean isRotate) {
    UVCCamera *camera = reinterpret_cast<UVCCamera *>(id_camera);
    if (LIKELY(camera)) {
...
//2022年5月17日16:33:29 设置机芯设置 参数
static jboolean
nativeSetMachineSetting(JNIEnv *env, jobject thiz, ID_TYPE id_camera, jint flag, jint value,
                        jint mark) {
    jboolean result = 0;
    UVCCamera *camera = reinterpret_cast<UVCCamera *>(id_camera);
    if (LIKELY(camera)) {
        result = camera->setMachineSetting(value, mark);
    }
    return result;
}
...
//2022年5月17日16:33:29 设置机芯设置 参数
static jfloat
nativeGetMachineSetting(JNIEnv *env, jobject thiz, ID_TYPE id_camera, jint flag, jint value,
                        jint mark) {
    jfloat result = 0;
    UVCCamera *camera = reinterpret_cast<UVCCamera *>(id_camera);
    if (LIKELY(camera)) {
...
static jbyteArray nativeGetByteArrayTemperaturePara(JNIEnv *env, jobject thiz,
    ...
    if (LIKELY(camera)) {
        ...
        LOGE("nativeGetByteArrayTemperaturePara:%d,%d,%d,%d,%d", para[1], para[5], para[9],
             para[13], para[17]);
    }
...
static jint nativeStartStopCapture(JNIEnv *env, jobject thiz,
                                   ID_TYPE id_camera, jint startStop) {
    ...
    UVCCamera *camera = reinterpret_cast<UVCCamera *>(id_camera);
    if (LIKELY(camera)) {
//        if(startStop)//1 start 0stop  //2021-8-11 16:20:46 吴长城  无需此接口
//        {
...
static void nativeSetAreaCheck(JNIEnv *env, jobject thiz,
                               ID_TYPE id_camera, jint isAreaChecked) {

    ENTER();
    UVCCamera *camera = reinterpret_cast<UVCCamera *>(id_camera);
    if (LIKELY(camera)) {
        camera->setAreaCheck(isAreaChecked);
    }
    EXIT();
}


static void nativeSetArea(JNIEnv *env, jobject thiz,
                          ID_TYPE id_camera, jintArray area) {

    ENTER();
    UVCCamera *camera = reinterpret_cast<UVCCamera *>(id_camera);

    if (LIKELY(camera)) {
        int length = env->GetArrayLength(area);
        //由于jint和int是一样的。所以用int* 接收.
        int *array = env->GetIntArrayElements(area, 0);
        camera->setArea(array, length);
        env->ReleaseIntArrayElements(area, array, 0);
    }
...
static void nativeLaWenKuan(JNIEnv *env, jclass clazz,
                            jlong id_camera, jfloat maxPercent, jfloat minPercent, jfloat maxValue,
                            jfloat minValue) {
...
static void nativeDisWenKuan(JNIEnv *env, jclass clazz,
                             jlong id_camera) {

    // TODO: implement nativeDisWenKuan()
    ENTER();
    UVCCamera *camera = reinterpret_cast<UVCCamera *>(id_camera);
    if (LIKELY(camera)) {
        camera->disWenKuan();
...
//added by wupei
static void nativeSetAddress(JNIEnv *env, jobject thiz, ID_TYPE id_camera, jstring path) {
    ENTER();
    UVCCamera *camera = reinterpret_cast<UVCCamera *>(id_camera);
    if (LIKELY(camera)) {
        const char *respath = env->GetStringUTFChars(path, 0);
        camera->setResourcePath(respath);
...
//长城添加:保存五秒数据
static void
nativeSaveFiveSecondsData(JNIEnv *env, jobject thiz, ID_TYPE id_camera, jstring externalPath) {
    ENTER();
    UVCCamera *camera = reinterpret_cast<UVCCamera *>(id_camera);
    if (LIKELY(camera)) {
        const char *respath = env->GetStringUTFChars(externalPath, 0);
        camera->setExternalPath(respath);
        env->ReleaseStringUTFChars(externalPath, respath);
    }
    EXIT();
}
...
static JNINativeMethod methods[] = {
        ...
        {"nativeSetMachineSetting",                 "(JIII)Z",                                        (void *) nativeSetMachineSetting},
        {"nativeGetMachineSetting",                 "(JIII)F",                                        (void *) nativeGetMachineSetting},
