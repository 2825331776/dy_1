...
//	LOGE("============D_saveData fileName=============  %s",fileName);
    ...
    int defineDataLen = oldFileLen - oldHeadLen - FFE2Maker_total_len;
    LOGE("=====剩下的字节数为：==%d,oldFileLen==%d=,FFD8和FFE0的长度oldHeadLen=%d,FFE2的总长度为：%d", defineDataLen,
         oldFileLen, oldHeadLen, FFE2Maker_total_len);
...
//    const char * testFilename = "/storage/emulated/0/DCIM/DYTCamera/testFile.jpg";
    ...
    if (fwp) {
        fwrite(pNewFileNoE2, 1, defineDataLen + oldHeadLen, fwp);
    }
    fclose(fwp);
...
/**************************自定义结构体数据********************************/
    ...
    fileHead.offset = 1640;//直接到红外图幅数据的标识
...
//    LOGE("===D_FILE_HEAD=========%d",sizeof(fileHead));
    ...
    unsigned short max, min;//图幅的最大值，最小值，差值
    mFrameImage->SearchMaxMin(adPicbuffer, frameWidth * frameHeight, &max, &min);

    D_IR_DATA_PAR irDataPar;
    irDataPar.ir_w = frameWidth;
    irDataPar.ir_h = frameHeight;

    irDataPar.raw_max = max;//动态获取
    irDataPar.raw_min = min;//动态获取
//    irDataPar.raw_max = 8000;
//    irDataPar.raw_min = 5000;

    irDataPar.pointSize = 2;
    irDataPar.typeAndSize = (0 << 4) | 4;;
...
//    LOGE("===D_GPS_INFO==此时len=======%d",len);
    ...
    // 图形说明 + 7个byte预留 共计：12byte 加详细信息（此时无详细 点线矩形等）。
    D_SHAPE_INFO dShapeInfo;
    dShapeInfo.pointSize = 0;
    dShapeInfo.lineSize = 0;
    dShapeInfo.recSize = 0;
