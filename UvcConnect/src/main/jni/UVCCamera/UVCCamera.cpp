...
//			libusb_set_debug(mContext->usb_ctx, LIBUSB_LOG_LEVEL_DEBUG);
        ...
        // カメラ機能フラグをクリア 		清除相机功能标志
        clearCameraParams();
        fd = dup(fd);
        // 指定したvid,idを持つデバイスを検索, 見つかれば0を返してmDeviceに見つかったデバイスをセットする(既に1回uvc_ref_deviceを呼んである)
        //搜索具有指定vid，id的设备（如果找到），返回0并在mDevice中设置找到的设备（已经称为uvc_ref_device一次）
...
int UVCCamera::getByteArrayPicture(uint8_t *frame) {
    ...
    int result = EXIT_FAILURE;
    if (mDeviceHandle) {
//		return mPreview->getByteArrayPicture(frame);吴长城
    }
    RETURN(result, int);
}
...
int UVCCamera::stopCapture() {
    ENTER();
    int result = EXIT_FAILURE;
    if (mPreview) {
//		result = mPreview->stopCapture();//吴长城
    }
    RETURN(result, int);
}

int UVCCamera::startCapture() {
    ENTER();
    int result = EXIT_FAILURE;
    if (mPreview) {
//		result = mPreview->startCapture();//吴长城
    }
    RETURN(result, int);
}
...
void UVCCamera::changePalette(int typeOfPalette) {
    ENTER();
    if (mFrameImage) {
        mFrameImage->changePalette(typeOfPalette);
    }
    EXIT();
}
 ...
 * 保存TinyC 相机参数 指令
 */
void UVCCamera::setTinySaveCameraParams() {
    ENTER();
    if (mPreview) {
        mPreview->setTinySaveCameraParams();
    }
    EXIT();
}
...
/* void UVCCamera::setTempRange(int range)
 {
     ...
     if (mPreview) {
//   		mPreview->setTempRange(range);吴长城
     }
     EXIT();
 }*/
...
void UVCCamera::setShutterFix(float mShutterFix) {
    ENTER();
    if (mPreview) {
//      		mPreview->setShutterFix(mShutterFix);吴长城
    }
    EXIT();
}
...
void UVCCamera::setExternalPath(const char *path) {
    ENTER();
    if (mDeviceHandle) {
//			mPreview->setExternalPath(path);
    }
    EXIT();
}
...
void UVCCamera::setCameraLens(int mCameraLens) {
    ENTER();
    if (mPreview) {
//   		mPreview->setCameraLens(mCameraLens);吴长城
    }
    EXIT();
}
...
int UVCCamera::setCaptureDisplay(ANativeWindow *capture_window) {
    ...
    int result = EXIT_FAILURE;
    if (mPreview) {
//		result = mPreview->setCaptureDisplay(capture_window);吴长城
    }
    RETURN(result, int);
}
...
int UVCCamera::internalSetCtrlValue(control_value_t &values, int16_t value,
    ...
    int ret = update_ctrl_values(mDeviceHandle, values, get_func);
    if (LIKELY(!ret)) {    // 设置的值是否在最大和最小值之间,否则设置的值 为最大或者最小值
        value = value < values.min
                ? values.min
                : (value > values.max ? values.max : value);
        set_func(mDeviceHandle, value);
...
int UVCCamera::getWhiteBlance() {
    ENTER();
    if (mPUSupports & PU_WB_TEMP) {
        int ret = update_ctrl_values(mDeviceHandle, mWhiteBlance,
                                     uvc_get_white_balance_temperature);
        if (LIKELY(!ret)) {    // 正常に最小・最大値を取得出来た時
            uint16_t value;
...
int UVCCamera::getWhiteBlanceCompo() {
    ENTER();
    if (mPUSupports & PU_WB_COMPO) {
        int ret = update_ctrl_values(mDeviceHandle, mWhiteBlanceCompo,
                                     uvc_get_white_balance_component);
        if (LIKELY(!ret)) {    // 正常に最小・最大値を取得出来た時
...
//			}
//		}
        ret = uvc_diy_communicate(mDeviceHandle, 0xc1, 0x44, 0x0078, 0x1d08, reData, sizeof(reData),
                                  1000);
