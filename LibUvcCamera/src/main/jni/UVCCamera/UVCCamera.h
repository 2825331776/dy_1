...
void whenShutRefresh() ;
	...
	int setPreviewSize(int width, int height, int min_fps, int max_fps, int mode, float bandwidth = DEFAULT_BANDWIDTH,int currentAndroidVersion=0);
...
/*	int updateFocusSimpleLimit(int &min, int &max, int &def);
	...
	void laWenKuan(float max, float min);
	void disWenKuan();
