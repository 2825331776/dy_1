...
import com.dyt.wcc.libuvccamera.R;
...
public class CameraDialog extends DialogFragment {
    ...
    public Dialog onCreateDialog(final Bundle savedInstanceState) {
		...
		builder.setPositiveButton("OK", mOnDialogClickListener);
		builder.setNegativeButton("Cancel", mOnDialogClickListener);
		...
		public View getView(final int position, View convertView, final ViewGroup parent) {
			...
			if (convertView instanceof CheckedTextView) {
				...
				((CheckedTextView)convertView).setText(
					//String.format("Xtherm:(%x:%x:%s)", device.getVendorId(), device.getProductId(), device.getDeviceName()));
						String.format("%s:%s", device.getProductName(),device.getSerialNumber()));
			}
			return convertView;
		}
	}
