...
import static com.serenegiant.glutils.ShaderConst.HEADER_2D;
...
public class GLDrawer2D1 implements IDrawer2dES21 {
	 ...
	 */
	public static final  String      VERTEX_SHADER1              = SHADER_VERSION + "uniform mat4 uMVPMatrix;\n" +                // モデルビュー変換行列
	...
	// フラグメントシェーダー
	public static final  String      FRAGMENT_SHADER_SIMPLE_OES1 = SHADER_VERSION + HEADER_OES + "precision mediump float;\n" + "uniform samplerExternalOES sTexture;\n" + "uniform sampler2D sTexture2D;\n" + "varying highp vec2 vTextureCoord;\n" + "void main() {\n" + "  lowp vec4 TextureColor = texture2D(sTexture, vTextureCoord);\n" + "  lowp vec4 TextureColor2D = texture2D(sTexture2D, vTextureCoord);\n" + "  gl_FragColor=TextureColor*(1.0-TextureColor2D.a)+TextureColor2D*TextureColor2D.a;\n" + "}";
	public static final  String      FRAGMENT_SHADER_SIMPLE1     = SHADER_VERSION + HEADER_2D + "precision mediump float;\n" + "uniform sampler2D sTexture;\n" + "varying highp vec2 vTextureCoord;\n" + "void main() {\n" + "  gl_FragColor = texture2D(sTexture, vTextureCoord);\n" + "}";
	...
	private static final String      FRAGMENT_SHADER_BASE1       = SHADER_VERSION + "%s" + "precision mediump float;\n" + "varying vec2 vTextureCoord;\n" + "uniform %s sTexture;\n" + "void main() {\n" + "    gl_FragColor = texture2D(sTexture, vTextureCoord);\n" + "}\n";
	...
	private Boolean isDv      = BaseApplication.deviceName.contentEquals("T3");
	// 頂点シェーダー

	/**
	 * 构造函数
	 * 必须在启用 GL 上下文/ EGL 渲染上下文的情况下调用
	 *
	 * @param isOES 使用外部纹理 (GL_TEXTURE_EXTERNAL_OES) 时为真。
	 *              普通 2D 纹理为 False
	 */
	public GLDrawer2D1 (final boolean isOES) {
		this(VERTICES, TEXCOORD, isOES);
		//        this(VERTICES, MyApp.deviceName.contentEquals("T3") || MyApp.deviceName.contentEquals("DL13")? TEXCOORD1:TEXCOORD, isOES);
	}
	 ...
	 * 必须在启用 GL 上下文/EGL 渲染上下文的情况下调用
	 *
	 * @param vertices 顶点坐标，8 个浮点数 = (x, y) x 4 对
	 * @param texcoord 纹理坐标，8 个浮点数 = (s, t) x 4 对
	 * @param isOES    使用外部纹理 (GL_TEXTURE_EXTERNAL_OES) 时为真。
	 *                 普通 2D 纹理为 False
	 */
	public GLDrawer2D1 (final float[] vertices, final float[] texcoord, final boolean isOES) {
	...
	@Override
	public synchronized void draw (final int[] texIds, final float[] tex_matrix, final int offset, Bitmap bitmap) {
