...
//		if (DEBUG) Log.v(TAG, "initTex:target=" + texTarget);
    ...
    public static int[] initTexes(int[] para) {
        ...
        final int n = para[0] > textureUnits[0]
                ? textureUnits[0] :  para[0];
        for (int i = 0; i < n; i++) {
            texIds[i] = GLHelper1.initTex(para[4*i+1],TEX_NUMBERS[i],
                    para[4*i+2], para[4*i+3], para[4*i+4]);
        }
        return texIds;
    }
    /**
     ...
     * @return
     */
    public static int[] initTexes(@NonNull final int n,
    ...
    public static int[] initTexes(@NonNull final int[] texIds,
        ...
        for (int i = 0; i < n; i++) {
            texIds[i] = GLHelper1.initTex(texTarget,TEX_NUMBERS[i],
    ...
    public static int[] initTexes(@NonNull final int[] texIds,
        ...
        final int n = texIds.length > textureUnits[0]
                ? textureUnits[0] : texIds.length;
        for (int i = 0; i < n; i++) {
            texIds[i] = GLHelper1.initTex(texTarget, texUnit,
                    min_filter, mag_filter, wrap);
        }
        return texIds;
    }
