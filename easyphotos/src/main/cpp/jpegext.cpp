...
//		printf("打开test.jpg失败\n");
	...
	int flag = 0;
	while (true) {
        //当flag不等于 0 时，nOriginalHeadCopyLength 的值为 FFE0的末尾。
		if (flag == 0) {
			...
			if (pOriginFileBuffer[++nOriginalHeadCopyLength] == 0xFF && pOriginFileBuffer[++nOriginalHeadCopyLength] == 0xE1) {
				...
				break;//保证 nOriginalHeadCopyLength 在E0 的末尾。
			}
		}
	}
