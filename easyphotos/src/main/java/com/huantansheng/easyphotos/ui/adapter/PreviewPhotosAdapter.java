...
public class PreviewPhotosAdapter extends RecyclerView.Adapter<PreviewPhotosAdapter.PreviewPhotosViewHolder> {
	...
	public PreviewPhotosAdapter (Context cxt, ArrayList<Photo> photos, OnClickListener listener) {
		...
		touchGestureListener = new DoodleOnTouchGestureListener();
		...
		doodleTouchDetector = new DoodleTouchDetector(cxt, touchGestureListener);
	}
	...
	public void onBindViewHolder (@NonNull final PreviewPhotosViewHolder holder, int position) {
		...
		if (type.contains(Type.VIDEO)) {
			...
			holder.doodleView.setVisibility(View.GONE);
			...
			//			Glide.with(holder.ivPhotoView.getContext()).load(frameBitmap).into(holder
			//			.ivPhotoView);
			...
			holder.ivPhotoView.setVisibility(View.GONE);
			holder.doodleView.setVisibility(View.VISIBLE);

			if (photos.get(position).gestureDetectorAble) {
				holder.doodleView.setGestureRecognitionAble(true);
			} else {
				holder.doodleView.setGestureRecognitionAble(false);
			}

			holder.doodleView.setBitmap(photos.get(position).path);
			//			if (ratio > 2.3) {
		...
		holder.doodleView.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick (View v) {
				listener.onPhotoClick();
			}
		...
		//		holder.doodleView.setScale(1f);

		//		holder.doodleView.setOnScaleChangeListener(new OnScaleChangedListener() {
		//			@Override
	...
	public class PreviewPhotosViewHolder extends RecyclerView.ViewHolder {
		...
		PreviewPhotosViewHolder (View itemView) {
			...
			doodleView = itemView.findViewById(R.id.doodle_item_view);
