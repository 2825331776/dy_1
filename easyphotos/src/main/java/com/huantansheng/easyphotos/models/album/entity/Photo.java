...
public class Photo implements Parcelable {
    ...
    //是否 item 单独去识别手势
	public               boolean gestureDetectorAble = false;


	public Photo (String name, Uri uri, String path, long time, int width, int height,
                  int orientation, long size, long duration, String type, boolean gestureAble) {
		this.name = name;
		this.uri = uri;
		this.path = path;
		this.time = time;
		...
		this.gestureDetectorAble = gestureAble;
	}
	...
	public String toString () {
		return "Photo{" + "name='" + name + '\'' + ", uri='" + uri.toString() + '\'' + ", path='" + path + '\'' + ", time=" + time + '\'' + ", minWidth=" + width + '\'' + ", minHeight=" + height + ", orientation=" + orientation + ",gestureDetectorAble" + gestureDetectorAble + '}';
	}
	...
	public void writeToParcel (Parcel dest, int flags) {
		...
		dest.writeByte(this.gestureDetectorAble ? (byte) 1 : (byte) 0);
	}
	...
	protected Photo (Parcel in) {
		...
		this.gestureDetectorAble = in.readByte() != 0;
	}
