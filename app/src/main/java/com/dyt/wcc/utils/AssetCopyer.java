...
public class AssetCopyer {
	...
	private static void copyAssetsToDst (Context context, String srcPath, String dstPath) {
		try {
			...
			Log.e(TAG, "copyAssetsToDst: ======================fileNames ======size ====" + Arrays.toString(fileNames));
			if (fileNames.length > 0) {
				File file = new File(dstPath);
				file.mkdirs();
				for (String fileName : fileNames) {
					//                    Log.e(TAG, "copyAssetsToDst: fileName======== > " + fileName);
					if ("".equals(srcPath) && fileName.contains("dat") || (fileName.contains(BuildConfig.CONFIGS_NAME))
							|| (fileName.contains("SLReadMe"))) {
...
//				Log.e(TAG, "copyAssetsToDst: ==========getPackageName=============" + context.getPackageName());
