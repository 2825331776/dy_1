package com.dyt.wcc.dypir.ui.preview.record;
 ...
 * <p>Create Date：2021/9/7  16:17 </p>
 * <p>Description：@todo describe         </p>
 * <p>PackgePath: com.wcc.dytfourbie.main.preview.service     </p>
 */
public class MediaProjectionService extends Service {
	...
	private void createImageReader () {
		...
		virtualDisplayImageReader = mediaProjection.createVirtualDisplay("ScreenCapture", width, height, densityDpi, DisplayManager.VIRTUAL_DISPLAY_FLAG_AUTO_MIRROR, imageReader.getSurface(), null, null);
	}
	...
	private void createMediaRecorder () {
		...
		// 创建保存文件
		mediaFile = new File(DYConstants.PIC_PATH, FileUtils.getDateName("DYTVideo") + ".mp4");
		...
		if (virtualDisplayMediaRecorder == null) {
			virtualDisplayMediaRecorder = mediaProjection.createVirtualDisplay("MediaRecorder", width, height, densityDpi, DisplayManager.VIRTUAL_DISPLAY_FLAG_AUTO_MIRROR, mediaRecorder.getSurface(), null, null);
		} else {
	...
	public void createVirtualDisplay (int resultCode, Intent data, DisplayMetrics displayMetrics, boolean isScreenCaptureEnable, boolean isMediaRecorderEnable) {
		...
		if (isScreenCaptureEnable) {//如果需要屏幕截图,则初始化相关成员变量
			createImageReader();
		}
	}
	...
	public void capture (ScreenCaptureCallback callback) {
		if (!isScreenCaptureEnable) {
			callback.onFail();
			Log.e(TAG, "onError:  ===11=== ");
			return;
		}
		if (imageReader == null) {
			callback.onFail();
			Log.e(TAG, "onError:  ===12=== ");
			return;
		}
		if (!isImageAvailable) {
			callback.onFail();
			Log.e(TAG, "onError:  ===13=== ");
			return;
		}
	...
	public void startRecording (MediaRecorderCallback callback) {
		...
		createMediaRecorder();
		mediaRecorderCallback.onStartCounting();
	...
	public void stopRecording () {
		...
		mediaRecorderCallback.onStopCounting();
