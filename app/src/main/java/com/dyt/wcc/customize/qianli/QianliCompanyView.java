...
import com.dyt.wcc.databinding.PopCompanyQianliBinding;
 ...
 */
public class QianliCompanyView extends CustomizeCompany {
	/**
	 * 设置显示的公司布局
	...
	public View getCompanyView (Context context) {
		...
		return LayoutInflater.from(context).inflate(R.layout.pop_company_qianli,null);
	}

	@Override
	public void initListener (View view) {
		PopCompanyQianliBinding qianliBinding = DataBindingUtil.bind(view);
		//根据是中文还是因为 区分是主营/邮箱反馈
		Objects.requireNonNull(qianliBinding).tvCompanyWebsite.getPaint().setFlags(Paint.UNDERLINE_TEXT_FLAG);
		qianliBinding.tvAboutContactUsEmail.setOnClickListener(v19 -> {

			Intent emailIntent = new Intent(Intent.ACTION_SENDTO);
			emailIntent.setData(Uri.parse(mContext.getResources().getString(R.string.contactus_email_head) +
					mContext.getResources().getString(R.string.about_contactEmail_content_qianli)));
