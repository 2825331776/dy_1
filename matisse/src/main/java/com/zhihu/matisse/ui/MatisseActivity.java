    ...
    public void onClick(View v) {
        if (v.getId() == R.id.button_preview) {
            ...
            if (!selectedUris.isEmpty()) {
                for (String list : selectedPaths) {
                    boolean del = deleteFile(list);
                    if (!del)
                        result = del;
                }
                for (Uri list : selectedUris) {
                    String uriPath = list.toString();
                    int deleteResult = 0;
                    deleteResult = getContentResolver().delete(list, null, null);//删除系统缩略图
                }
                if (result) {
                    switch (albumSelected) {
                        case 0:
                            albumSelected = 1;
                            break;
                        case 1:
                    ...
                    new Handler().postDelayed(new Runnable() {
                        public void run() {
                            mAlbumsSpinner.setSelection(MatisseActivity.this, albumSelected);
                            mAlbumCollection.setStateCurrentSelection(albumSelected);
                            mAlbumsAdapter.getCursor().moveToPosition(albumSelected);
                            ...
                            Toast.makeText(MatisseActivity.this, "删除成功！", Toast.LENGTH_SHORT).show();
                        }
                    }, 500);
                }
            }
